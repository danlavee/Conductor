...
3. **Register as `conductor-maintenance`** with exactly this responsibility string, unchanged, every time: `conductor <agent> register "repo upkeep: archive stale Conductor history, keep collaboration/rules seeded, never delete data"`. Any other wording under this name is rejected by the registry, and a different wording breaks every later maintenance join.
...
6. **Hand off.** Tell the user onboarding is done: `conductor-maintenance` is registered and watching, and every agent that registers from here on receives the seeded rules automatically, without needing to read this file or the rules file themselves. Continue as an ordinary registered agent per the rest of `SKILL.md`.
...
**Registering `conductor-maintenance` with paraphrased or shortened responsibility text.** The registry treats a changed responsibility under an existing name as a conflict, not an update — it will reject the join outright, not silently accept the new wording.
