...
- `conductor migrate <source-root> <destination-root>` requires the source to be behind a completed cutover freeze barrier. It then auto-detects protocol v1, v2, or v3 and runs the matching migration into a fresh, distinct destination: v1 produces a current v4 root, v2 translates legacy wire-shape drift into v3, and v3 compacts topic history into v4.
...
- Each agent's watch holds its own process-ownership lock; a second concurrent watcher for the same identity returns `LOCKED` naming the exact PID that already holds it, so the right process can be stopped instead of guessed at. This is what makes restoration blind-safe: any number of attempts, from any source, converge on one live stream, and a redundant attempt fails fast instead of double-delivering.
- `status` reports whether an identity is currently wakeable, without acquiring the ownership lock and without disturbing a live stream. It is derived from observable state, never from a caller's assertion.
- Cutover coordination lives in a sibling control directory outside the versioned root. Normal operations take a crash-released admission lease. `freeze` closes admission and drains those leases before declaring `frozen`; from that barrier through replacement activation, no normal operation may read or write the root. A watcher that crosses the barrier polls only this external control state and emits one `conductor-replaced` activation with no delta or acknowledgment. Missing or corrupt control state fails closed after freeze observation, and a timeout never reopens admission.
...
- `watch` is one continuous stream: it emits a delivery, acknowledges it as accepted, and continues waiting. Semantics are identical on every host; nothing vendor-specific reaches the core. `--once` emits a single delivery and exits, for hosts whose wake is the watch process completing.
- One delivery resolves the current pending backlog up to a shared `DefaultReadLimit` budget (`{"deliveries": [...], "remaining": N, "default_read_limit": 20}`), not just the next signal. Consecutive-or-not pending `update` signals for the same topic are grouped into one resolved delta instead of one `get` apiece; unrelated topics, joins, and leaves get separate entries. The first delivery is always included whole even if it alone exceeds the budget. A signal left out, or left short by a delta cap, is never touched: it stays pending and is counted in `remaining`, ready to resolve fresh in the next delivery.
